...
public class UnitService: IUnitService
{
    ...
    public async Task<GetUnitsResponse> GetUnitAsync(GetUnitRequest request)
    {
        var query = request.UnitId.HasValue
            ? _db.Units.Where(x => x.Id == request.UnitId)
            : _db.Units.AsQueryable();

        var result = await query.GetPageAsync<GetUnitsResponse, Unit, UnitModel>(request, x =>
            new UnitModel
            {
                Id = x.Id,
                Name = x.Name
